...
public class ManagerJdbcDao implements ManagerDao{
    ...
    public int insert(Manager manager) {
        ...
        try {
            ...
            String sql="     insert into p1_memb(name,email,pwd,tel,cdt)"
                            + " values('"+ manager.getName()+ "' , '"+manager.getEmail()
                            + "',password('" + manager.getPassword() + "')"
                            + ",'"+manager.getTel()
                            + "',now()) ";
            
            // p1_memb 테이블에 회원 기본정보를 입력 한 후
            // 자동으로 생성된 휘원 번호를 리턴 받는다.
    ...
    public Manager findByEmail(String email) {
        ...
        try {
            ...
            rs=stmt.executeQuery("     select m.mno, m.name, m.email, m.tel, mr.posi         " + 
                                 "       from p1_mgr mr                        " + 
                                 "       join p1_memb m                        " + 
                                 "         on mr.mrno=m.mno                    " +
                                 "        where m.email='"+email+"'");
