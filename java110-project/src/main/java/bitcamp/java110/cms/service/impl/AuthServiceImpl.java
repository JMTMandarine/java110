...
public class AuthServiceImpl implements AuthService {
  ...
  public Member GetFacebookMember(String accessToken, String type) {
    ...
    // Facebook의 Graph API 실행 요청하기
    // => HTTP 요청을 할 떄 스프링에서 제공하는 RestTemplate을 사용하라!
    ...
    // 현재 서버에 가입하지 않았다면
    // 페이스북 기본정보를 가지고 자동으로 회원 등록한다. 
    Member newbie = createMember(
        type, 
  ...
  private Member createMember(String memberType, String name, String email, String password) {
    Member member = null;
    if (memberType.equals("manager")) {
      member = new Manager();
    } else if (memberType.equals("student")) {
