...
public class InitServlet extends HttpServlet {
    ...
    // service() 또는 doGet(), doPost() 등의 메서드를 구현하지 않는다.
    // 클라이언트 요청을 처리하는 다른 서블릿들이 사용할 공용 자원을 준비하는 일을 함.
    // 그래서 service(), doGet, doPost를 구현하지 않는다.
    // 클라이언트가 요청하지 않아도 이 서블릿 객체가 생성되고 
    // init()가 실행되어야 하기 때문에 loadOnStartup 속성을 추가한다.
}
