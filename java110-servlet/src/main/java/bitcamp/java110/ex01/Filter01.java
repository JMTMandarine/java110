...
public class Filter01 implements Filter{
    ...
    public Filter01() {
        System.out.println("Filter01() 생성자 호출!");
    }
    ...
    public void doFilter(ServletRequest request,
        ...
        // 이때 이 메서드가 호출된다.
        // 서블릿을 실행하기 전,후에 수행해야 할 작업이 있다면 이 메서드에서 실행 
        // 예)
        // - 클라이언트가 암호화해서 보낸 데이터 서블릿이 사용할 수 있도록 암호 해제
        // - 클라이언트가 압축해서 보낸 데이터를 서블릿이 사용할 수 있도록 합축 해제
        // - 클라이언트의 요청에 대해 기록 남기기
        // - 클라이언트가 특별한 형식으로 보낸 데이터를 파싱하여 서블릿이 사용하기 쉽게 변환
        // - 클라이언트가 요청한 서블릿의 실행 권한이 있는지 검사
        // - 클라이언트가 로그인 한 사용자인지 검사
        
        System.out.println("Filter01.doFilter() ----before");
        // -> 다음 필터가 있다면 실행하고 없다면 서블릿을 호출
        chain.doFilter(request, response);
        // 위 메서드를 호출하지 않으면 다음 필터나 서블릿을 실행하지 않음
        // 따라서 빼면 안됨.
