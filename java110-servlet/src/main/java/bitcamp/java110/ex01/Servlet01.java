...
// 서블릿 생성시 컨테이너에 등록해야함
// URL을 지정해야 클라이언트가 호출 가능 (URL은 / 로 시작)
// 클라이언트는 다음과 같이 요청함
...
// 3) 웹 애플리케이션의 경로를 지정하지 않았을 경우,(프로젝트이름을 사용한다)
//  http://localhost/8888/java110-servlet/ex01/Servlet01
//
// 서블릿을 새로 만든경우, 서블릿 컨테이너를 다시 시작해야 적용됨
...
public class Servlet01 implements Servlet {
    ...
    public Servlet01() {
        System.out.println("Servlet01() 서블릿의 탄생 우왘");
    ...
    public String getServletInfo() {
        // 서블릿을 싱행 중에 직접 호출할 일은 없다.
        // 주로 서블릿 컨테이너 관리 메뉴에서 서블릿 정보를 출력할 때 
...
//
// 주의!
// - 서블릿 객체는 클래스 당 한 개만 생성된다.(요청할 때마다 생성되는 것이 아니다.)
// - 클라이언트마다 객체가 생성되는 것이 아니라 한 객체를 사용하는 것이기 때문에,
//   객채의 인스턴스 변수를 공유하게 된다.
// - 클라이언트 마다 구분해서 값을 저장해야한다면
