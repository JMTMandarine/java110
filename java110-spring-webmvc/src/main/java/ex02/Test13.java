...
public class Test13 {
    ...
    @RequestMapping("m1")  
    public String m1(
            @RequestParam(name="name") String name,  // name과 value는 상관없음
            @RequestParam(value="age") int age, // 생략안한 예
    ...
    @RequestMapping("m2")  
    public String m2(
            @RequestParam(name="name", required=false) String name,  // name과 value는 상관없음
            @RequestParam(value="age", required=false) int age, // 생략안한 예
            @RequestParam(value="tel", required=false) String tel, //value는 생략 가능
            Model model
            ) {
        model.addAttribute("name", name);
        model.addAttribute("age", age);
    ...
    public String m3(
            @RequestParam(name="name", required=false) String name,  // name과 value는 상관없음
            @RequestParam(value="age", defaultValue="20") int age, // 생략안한 예
            @RequestParam(value="tel", required=false) String tel, //value는 생략 가능
            Model model
            ) {
        model.addAttribute("name", name);
