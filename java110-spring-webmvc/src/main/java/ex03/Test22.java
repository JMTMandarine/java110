...
public class Test22 {
    ...
    // 페이지 컨트롤러에 @ResController를 붙이면
    // 프론트 컨트롤러는 request handler의 리턴값을 클라이언트로 바로 출력한다.
    // 1) 리턴 타입이 String, primitive type일 경우,
    //    => 문자열로 변환한 후 출력한다.
    // 2) 리턴 타입이 기타 객체일 경우,
    ...
    //    => "Http Message"는 클라이언트가로부터 받은 데이터, 
    //       클라이언트로 보내는 데이터를 가리킨다.
    // 2) 만약 HttpMessageConverter 구현체를 못 찾았다면?
    //    => 리턴값을 바꿀 수 없기 때문에 실행 오류가 발생한다!
    //
    // HttpMemssageConverter 구현체는 어떻게 추가하는가?
    // => 스프링 프레임워크는 내부적으로 기본 데이터 변환기로 다음 두 개의 클래스를 사용한다.
    ...
    //      "google-gson" 라이브러리
    //
    // => 따라서, @RestController를 사용하려면 위의 두개의 라이브러리 중 하나는
    //    반드시 포함해야 한다.
    //
 ...
 // 출력하는 데이터가 application/json이라고 프론트 컨트롤러에게 알려주면,
    ...
    // RequestController일 경우 리턴 타입이 String이라면, 그대로 출력한다.
    @RequestMapping("m4")
    ...
    // RequestController일 경우 리턴 타입이  primitive type이라면, 그대로 출력한다.
    @RequestMapping("m5")
    ...
    // @Reqeustbody를 생략하면 안된다!
    // => 생략하면, 
    //    클라이언트가 보낸 데이터가 일반 application/x-www-form-urlencoded 형식의
    //    변수=값&변수=값&... 형식의 데이터로 간주하고 Car 객체에 값을 넣으려고 할 것이다.
    ...
    // 빈 맵 객체를 넘겨줄 것이다.
    // Map변수에 JSON 데이터를 받고 싶다면, 변수 앞에 @RequestBody를 명시해야 한다.
    // 이 경우 생략하면 안된다.
    // 
    @RequestMapping("m9")
